...
export default function BillSelector({values, setFieldValue, errors, touched, index, factura, dataBills, setFieldTouched, setFieldError, cargarFraccionFactura, hasShownModal,       // Recibido del padre
  setHasShownModal}) {
    const [showConfirmationModal, setShowConfirmationModal] = useState(false);
    const [selectedFacturaWithCode, setSelectedFacturaWithCode] = useState(null);
    const [pendingSelection, setPendingSelection] = useState(null);
const validarIntegracionFacturas = (facturas, facturaActual, index) => {
    // 1. Si la factura actual no tiene billId (no está seleccionada todavía) → permitir
    if (!facturaActual.billId?.trim()) {
        return { valido: true };
    }

    // 2. Filtrar solo facturas que ya están seleccionadas (tienen billId)
    const facturasSeleccionadas = facturas.filter(
        f => f.billId?.trim() && f !== facturaActual
    );

    // 3. Verificar si hay facturas seleccionadas sin código
    const hayFacturasSinCodigo = facturasSeleccionadas.some(
        f => !f.integrationCode?.trim()
    );

    // 4. Verificar si la factura actual tiene código
    const facturaActualTieneCodigo = facturaActual.integrationCode?.trim();

    // Caso 1: Si hay facturas sin código y la actual tiene código → NO PERMITIDO
    if (hayFacturasSinCodigo && facturaActualTieneCodigo) {
        return {
            valido: false,
            mensaje: "No se puede agregar factura con código cuando hay facturas sin código"
        };
    }

    // Caso 2: Si hay facturas con código, verificar que coincidan
    const facturasConCodigo = facturasSeleccionadas.filter(
        f => f.integrationCode?.trim()
    );

    if (facturasConCodigo.length > 0 && facturaActualTieneCodigo) {
        const primerCodigo = facturasConCodigo[0].integrationCode;
        if (facturaActual.integrationCode !== primerCodigo) {
            return {
                valido: false,
                mensaje: `El código debe coincidir con "${primerCodigo}"`
            };
        }
    }
    ...
    const handleConfirmIntegrationCode = () => {
        ...
        // Filtrar facturas con el mismo integrationCode de dataBills.data
        const filteredIntegrationCodeBills = dataBills?.data.filter(
            (f) => f.integrationCode === selectedFacturaWithCode.integrationCode
        );
        
        // Actualizar las opciones disponibles en el selector
        setFieldValue('takedBillsBeforeFilterIntegration', values.takedBills);
        setFieldValue('takedBills', filteredIntegrationCodeBills);
        
        // Procesar la selección pendiente
        if (pendingSelection) {
            processBillSelection(pendingSelection);
        }
    };

    const handleCancelIntegrationCode = () => {
        setShowConfirmationModal(false);
        setPendingSelection(null);
        setSelectedFacturaWithCode(null);
        setFieldValue(`facturas[${index}].factura`, null);
    };

    const processBillSelection = async (newValue) => {
        const selectedFactura = dataBills?.data.find(f => f.billId === newValue.value);
        if (!selectedFactura) return;
   // Validación antes de cualquier modificación
    const validacion = validarIntegracionFacturas(
        values.facturas, 
        selectedFactura, 
        index
    );
    ...
    if (!validacion.valido) {
        ...
        const inversionistaSeleccionado = factura.nombreInversionista;
            ...
            if (
                ...
                if (factura.idCuentaInversionista) {
                    ...
                    if(facturasMismoInversionista.length > 1) {
                        // 3. Calcular total de presentValueInvestor
                        ...
                        montoDisponibleFinal = factura.montoDisponibleInfo - totalPresentValue - totalGm - presentValueInvestor;
                        ...
                        const montoDisponibleFinal = factura.montoDisponibleInfo - presentValueInvestor;
    ...
    return (
        <>
            <Autocomplete
                ...
                options={(values?.takedBills || [])
                    .filter((factura) => factura.currentBalance > 0)
                    .map((factura) => ({
                        label: `${String(factura.billId)}${factura.integrationCode ? `-${String(factura.integrationCode)}` : ''}`,
                        value: String(factura.billId),
                        id: String(factura.id),
                        integrationCode: factura.integrationCode || "",
                    }))
                }
                value={values.facturas[index]?.factura ? String(factura.billId) : null}
                ...
                onChange={async (event, newValue) => {
                    if (!newValue) {
                        ...
                        const algunaFacturaSinIntegrationCode = values.facturas.some(factura => 
                            !factura.integrationCode || factura.integrationCode.trim() === ""
                        );

                        console.log("¿Alguna factura no tiene integrationCode?", algunaFacturaSinIntegrationCode);

                        if(algunaFacturaSinIntegrationCode){
        ...
        // Validación después de deseleccionar
        const facturasRestantes = values.facturas.filter((_, i) => i !== index);
        const facturasConCodigo = facturasRestantes.filter(
            f => f.integrationCode?.trim() && f.integrationCode !== "no-code"
        );
        const facturasSinCodigo = facturasRestantes.filter(
            f => !f.integrationCode?.trim() || f.integrationCode === "no-code"
        );

        if (facturasConCodigo.length > 0 && facturasSinCodigo.length > 0) {
            toast.error("No se permiten mezclar facturas con y sin código de integración");
            // Opción: Forzar a que todas queden sin código
            facturasRestantes.forEach((f, i) => {
                if (i !== index) {
                    setFieldValue(`facturas[${i}].integrationCode`, "");
                }
            });
        }
                        return;
                    ...
                    if (selectedFactura?.integrationCode?.trim() && 
                        selectedFactura.integrationCode !== "") {
                        ...
                        if (!hasShownModal) {
                            setSelectedFacturaWithCode(selectedFactura);
                    ...
                    // Si no tiene integrationCode o ya mostramos el modal, procesar normalmente
                    await processBillSelection(newValue);
                }}
                renderInput={(params) => (
                    <TextField
                        {...params}
                        ...
                        helperText={` ${factura.isReBuy ? "Disponible Recompra" : "No disponible Recompra"}`}
                    />
                )}
            />
            ...
            <ModalIntegrationCode 
                ...
                integrationCode={selectedFacturaWithCode?.integrationCode}
                setIsFilterIntegrationActive={() => {}}
                selectedFactura={selectedFacturaWithCode}
                onConfirm={handleConfirmIntegrationCode}
                onCancel={handleCancelIntegrationCode}
            />
        </>
    );
}
