      ...
      <Autocomplete
        ...
        onChange={async (event, newValue) => {
          ...
          if (newValue && values.emitter?.data?.id !== newValue?.data?.id) {
            const brokerByClientFetch = await fetchBrokerByClient(
              newValue?.data.id
            );
            ...
            const discountRate =
              parseFloat(tasaDescuento?.data?.discount_rate) || 0;
            ...
            newValue &&
            values.emitter?.data?.id === newValue?.data?.id
          ) {
            const brokerByClientFetch = await fetchBrokerByClient(
              newValue?.data.id
            );
        ...
        renderInput={(params) => (
          <TextField
            ...
            helperText={
              touched.emitter && errors?.emitter ? errors?.emitter : " "
            }
          />
        )}
      />
